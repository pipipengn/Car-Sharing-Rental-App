...
func TestGetTrips(t *testing.T) {
	...
	for _, r := range rows {
		...
		if _, err := m.CreateTrip(c, &rentalpb.Trip{
	...
	for _, cc := range cases {
		t.Run(cc.name, func(t *testing.T) {
			trips, err := m.GetTrips(c, id.AccountID(cc.accountID), cc.status)
			if err != nil {
				t.Errorf("cannot got trips: %v", err)
			}

			if cc.wantCount != len(trips) {
				t.Errorf("incorrect result count; want: %d, got: %d", cc.wantCount, len(trips))
			}

			if cc.wantOnlyID != "" && len(trips) > 0 && cc.wantOnlyID != trips[0].ID.Hex() {
				t.Errorf("only_id incorrect; want: %q, got %q", cc.wantOnlyID, trips[0].ID.Hex())
			}
		})
	}
}
